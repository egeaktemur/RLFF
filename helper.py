...
def overlay_y_on_x(x, y=None, neu=False, num=10):
  x_ = x.clone()
  if neu:
    x_[:, -num:] = 1/num
  else:
    x_[:, -num:] = 0.0
    y_indices = y + (x.shape[1] - num)
...
def process_data(df, x_columns, y_column, quantiles=10, device=DEVICE):
  ...
  df[x_columns + [y_column]] = scaler.fit_transform(df[x_columns + [y_column]])
  df = df[x_columns + [y_column]]
  encoder = OneHotEncoder(sparse_output=False)
  df['Bins'] = pd.qcut(df[y_column], q=quantiles, labels=False, duplicates='drop')
  train_df, test_df = train_test_split(df, test_size=0.2, random_state=42)
  y_train, y_test = train_df['Bins'], test_df['Bins']
  y_train_enc, y_test_enc = encoder.fit_transform(y_train.values.reshape(-1, 1)), encoder.transform(y_test.values.reshape(-1, 1))
  y_train, y_train_num = move(y_train.values).long(), move(train_df[y_column].values)
  y_test, y_test_num   = move(y_test.values).long(), move(test_df[y_column].values)
