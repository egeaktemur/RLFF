...
from layers import FFLayer, FFClassifier, FFRegressor
...
class FFBaseNet(torch.nn.Module):
  ...
  def __init__(self, dims = [784, 2000, 2000, 2000, 2000], batch_size = 64, output_dim = 10,
    ...
    self.layers = nn.ModuleList([FFLayer(dims[d], dims[d + 1], learning_rate, dims[d + 1]*threshold_coeff).to(DEVICE) for d in range(len(dims) - 1)])
  ...
  def create_neg_batch(self, x, y):
    top2_preds   = torch.topk(self.get_goodness_per_label(x), 2, dim=1).indices
    x_neg_labels = torch.where(top2_preds[:, 0] == y, top2_preds[:, 1], top2_preds[:, 0])
...
class FFNet(FFBaseNet):
  def __init__(self, dims = [784, 2000, 2000, 2000, 2000], batch_size = 64, output_dim = 10,
    ...
    super().__init__(dims, batch_size, output_dim, learning_rate, threshold_coeff, model_name)
  ...
  def generate_x_neg(self, x_train_neu, y_train_enc, adaptive_x_neg, randomize_each_chapter):
    if adaptive_x_neg and self.use_classifier:
      top2_preds  = torch.topk(self.classifier(self.get_activations(x_train_neu)), 2, dim=1).indices
      labels = torch.where(top2_preds[:, 0] == y_train_enc, top2_preds[:, 1], top2_preds[:, 0])
      x_train_neg = overlay_y_on_x(x_train_neu, labels, False, self.output_dim).to(DEVICE)
    elif adaptive_x_neg and not self.use_classifier:
      for start_index, end_index in self.generate_batches(10):
        x_train_neg[start_index:end_index] = self.create_neg_batch(x_train_neu[start_index:end_index], y_train_enc[start_index:end_index])
  ...
  def test_model(self, x_test_neu, y_test, y_test_num = None, epochs = 0, train_time = 0):
    ...
    output = "Epochs: " + str(epochs) + " Training Time: " + str(train_time)
    goodness_test_ACC = self.evaluate(x_test_neu, y_test, method = "Goodness")
    output += "\n\tGoodness Test ACC: " + str(goodness_test_ACC)
    if self.use_classifier:
      classifier_test_ACC = self.evaluate(x_test_neu, y_test, method = "Classifier")
      output += "\n\tClassifier Test ACC: " + str(classifier_test_ACC)
    if self.use_regressor:
      regressor_test_AME = self.evaluate(x_test_neu, y_test_num, method = "Regression")
  ...
  def train_BP_Layers(self, x_train_neu, y_train, y_train_num, epoch, chapter, mini_epoch):
    ...
    for mini_epoch in range(epoch):
      ...
      for start_index, end_index in self.generate_batches():
        if self.use_regressor:
          self.regressor.train_layer(input[start_index:end_index], y_train_num[start_index:end_index])
        if self.use_classifier:
          self.classifier.train_layer(input[start_index:end_index], y_train[start_index:end_index])
    ...
    for chapter in range(split):
      ...
      for layer_index, layer in enumerate(self.layers):
        ...
        for mini_epoch in range(epochs//split):
          self.learning_rate_cooldown(self.current_epoch(chapter, mini_epoch))
          for start_index, end_index in self.generate_batches():
            layer_loss += layer.train_layer(input_pos[start_index:end_index], input_neg[start_index:end_index])
      ...
      if self.use_classifier or self.use_regressor:
        self.train_BP_Layers(x_train_neu, y_train, y_train_num, epochs//split, chapter, mini_epoch)
      if adaptive_x_neg or randomize_each_chapter:
        x_train_neg = self.generate_x_neg(x_train_neu, y_train, adaptive_x_neg, randomize_each_chapter)
      if chapter*(epochs//split) % report_every == 0 and chapter*(epochs//split) != self.epochs:
        self.test_model(x_test_neu, y_test, y_test_num, chapter*(epochs//split), time.time() - start_time)
