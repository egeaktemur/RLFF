...
def trainFF_MNIST(learning_rate = 0.01, threshold_coeff = 0.01, adaptive_x_neg = False, epochs = 100,
  ...
  train_loader_all, test_loader_all = MNIST_loaders(train_batch_size=60000, test_batch_size=10000)
  x_train_pos, x_train_neg, x_train_neu, y_train, x_test, x_test_neu, y_test, total_size = get_data(train_loader_all, test_loader_all)
  torch.manual_seed(seed)
  model_name = get_model_name(split, learning_rate, threshold_coeff, adaptive_x_neg, use_classifier, use_regressor, randomize_each_chapter)
  net = FFNet(dims = [784, 2000, 2000, 2000, 2000], model_name = model_name, use_classifier = use_classifier, batch_size = batch_size)
...
def trainFF_CIFAR(learning_rate = 0.01, threshold_coeff = 0.01, adaptive_x_neg = False, epochs = 100,
  ...
  if split is None:
    split = epochs
  torch.manual_seed(seed)
  train_loader_all, test_loader_all = CIFAR10_loaders(train_batch_size=50000, test_batch_size=10000)
  x_train_pos, x_train_neg, x_train_neu, y_train, x_test, x_test_neu, y_test, total_size = get_data(train_loader_all, test_loader_all)
  model_name = get_model_name(split, learning_rate, threshold_coeff, adaptive_x_neg, use_classifier, use_regressor, randomize_each_chapter)
  torch.manual_seed(seed)
